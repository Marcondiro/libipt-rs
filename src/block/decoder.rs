...
impl<'a, T> BlockDecoder<'a, T> {
    ...
    /// Returns Nosync if the decoder is out of sync.
    pub fn next(&mut self) -> Result<(Block, Status), (Block, PtError)> {
...
impl<'a, T> Iterator for BlockDecoder<'a, T> {
    type Item = Result<(Block, Status), (Block, PtError)>;

    fn next(&mut self) -> Option<Result<(Block, Status), (Block, PtError)>> {
        match self.next() {
            // eos to stop iterating
            Err((_, x)) if x.code() == PtErrorCode::Eos => None,
            x => Some(x),
        }
    }
}
